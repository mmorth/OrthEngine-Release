...
vec3 ComputeDirectionalLight(DirectionalLight directionalLight, vec3 normal, vec3 viewDirection)
{
    ...
    vec3 diffuse = directionalLight.diffuse * diffuseShading * vec3(texture(material.diffuseMap, TexCoords));
...
vec3 ComputePointLight(PointLight pointLight, vec3 normal, vec3 viewDirection, vec3 fragPos)
{
    ...
    vec3 ambient = attenuation * pointLight.ambient * vec3(texture(material.diffuseMap, TexCoords));
    vec3 diffuse = attenuation * pointLight.diffuse * diffuseShading * vec3(texture(material.diffuseMap, TexCoords));
...
vec3 ComputeSpotLight(SpotLight spotLight, vec3 normal, vec3 viewDirection, vec3 fragPos)
{
    ...
    // compute point light shading by combining results
    vec3 ambient = attenuation * intensity * spotLight.ambient * vec3(texture(material.diffuseMap, TexCoords));
    vec3 diffuse = attenuation * intensity * spotLight.diffuse * diffuseShading * vec3(texture(material.diffuseMap, TexCoords));
